...
export default function CartModal() {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 select-none dir-rtl animate-fade-in">
      {/* Backdrop */}
      <div
      ...
      {/* Modal / Drawer Box */}
      <div className="relative w-full max-w-lg bg-white dark:bg-dark-900 border border-stone-200 dark:border-white/10 rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[85vh] flex flex-col animate-slide-up overflow-hidden z-10">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-stone-200 dark:border-white/10">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-primary-600/10 text-primary-600 dark:text-primary-400 flex items-center justify-center">
              <ShoppingBag className="w-5 h-5" />
            </div>
            <div>
              <h2 className="font-bold text-lg text-stone-900 dark:text-white">سلة الطلب</h2>
          ...
          <button
            type="button"
            onClick={closeCart}
            className="w-8 h-8 rounded-full bg-stone-100 dark:bg-white/5 hover:bg-stone-200 dark:hover:bg-white/10 flex items-center justify-center text-stone-500 hover:text-stone-900 dark:hover:text-white transition-colors text-sm font-bold"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {cart.length === 0 ? (
            ...
            cart.map((line) => (
              <div
                key={line.variant_id}
                className="bg-stone-50 dark:bg-white/5 rounded-2xl p-4 flex items-start justify-between gap-3 border border-stone-200/80 dark:border-white/5"
              >
                ...
                <div className="flex items-center gap-0 border border-stone-200 dark:border-white/10 bg-white dark:bg-dark-800 rounded-xl overflow-hidden shadow-xs shrink-0">
                  <button
                    ...
                    className="px-2.5 py-1.5 text-stone-500 hover:bg-red-500/10 hover:text-red-600 transition-colors"
                  ...
                  <span className="px-2 text-xs font-black text-stone-900 dark:text-white min-w-[1.5rem] text-center tabular-nums">
                    {line.quantity}
                  </span>
                  <button
                    type="button"
                    onClick={() => updateQuantity(line.variant_id, line.quantity + 1)}
                    className="px-2.5 py-1.5 text-stone-500 hover:bg-primary-500/10 hover:text-primary-600 transition-colors font-bold text-sm leading-none"
        ...
        {/* Footer */}
        {cart.length > 0 && (
          <div className="p-5 border-t border-stone-200 dark:border-white/10 bg-stone-50/50 dark:bg-dark-950/50 space-y-3">
            ...
            <button
              ...
              className="w-full btn-primary py-3.5 rounded-2xl text-base font-bold flex items-center justify-center gap-2 shadow-lg shadow-primary-600/30 active:scale-[0.98]"
            >
